  ...
  electron.ipcMain.handle(
    ...
    async (_, {
      ...
      try {
          ...
          for (const imagePath of images) {
            try {
              ...
              requestBody.contents[0].parts.push({
                inlineData: {
                  mimeType: "image/jpeg",
                  // или 'image/png' в зависимости от формата
                  data: base64Image
                }
              });
            } catch (imageError) {
        ...
        const modelToUse = images.length > 0 ? "gemini-pro-vision" : "gemini-pro";
...
const HOTKEYS = {
  ...
  SEND_QUEUE: "CommandOrControl+Enter",
  // Отправка очереди в LLM
  CLEAR_QUEUE: "CommandOrControl+R",
  // Очистка очереди
  TOGGLE_COLLAPSE: "CommandOrControl+B",
...
function registerHotkeys(callbacks) {
  ...
  try {
    ...
    electron.globalShortcut.register(HOTKEYS.MOVE_DOWN, () => {
      callbacks.moveWindow("down");
    });
    registeredHotkeys.push(HOTKEYS.MOVE_DOWN);
    electron.globalShortcut.register(HOTKEYS.MOVE_LEFT, () => {
      callbacks.moveWindow("left");
    });
    registeredHotkeys.push(HOTKEYS.MOVE_LEFT);
    electron.globalShortcut.register(HOTKEYS.MOVE_RIGHT, () => {
      callbacks.moveWindow("right");
    });
    registeredHotkeys.push(HOTKEYS.MOVE_RIGHT);
    electron.globalShortcut.register(HOTKEYS.TOGGLE_COLLAPSE, () => {
    ...
    electron.globalShortcut.register(HOTKEYS.SEND_QUEUE, () => {
      electron.BrowserWindow.getAllWindows().forEach((window2) => {
        window2.webContents.send("hotkey-triggered", "send-queue");
      });
    });
    registeredHotkeys.push(HOTKEYS.SEND_QUEUE);
    electron.globalShortcut.register(HOTKEYS.CLEAR_QUEUE, () => {
      electron.BrowserWindow.getAllWindows().forEach((window2) => {
        window2.webContents.send("hotkey-triggered", "clear-queue");
      });
    });
  ...
  electron.ipcMain.handle("get-hotkeys", () => {
    return HOTKEYS;
  });
}
function unregisterAllHotkeys() {
  try {
    registeredHotkeys.forEach((hotkey) => {
      if (electron.globalShortcut.isRegistered(hotkey)) {
...
let requestsQueue = [];
function setupQueueService(mainWindow2) {
  const tempDir = path.join(electron.app.getPath("temp"), "voiceCopilot");
  if (!fs.existsSync(tempDir)) {
    fs.mkdirSync(tempDir, { recursive: true });
  }
  electron.ipcMain.handle("add-last-transcription-to-queue", () => {
    const lastTranscription2 = getLastTranscription();
    if (!lastTranscription2) {
      return { success: false, error: "Нет распознанного текста" };
    }
    const id = `text-${Date.now()}`;
    const isQuestion = detectQuestion(lastTranscription2.text);
    const queueItem = {
      id,
      type: "text",
      content: lastTranscription2.text,
      timestamp: Date.now(),
      isQuestion
    };
  ...
  electron.ipcMain.handle("add-clipboard-to-queue", () => {
    const clipboardText = electron.clipboard.readText().trim();
    if (!clipboardText) {
      const clipboardImage = electron.clipboard.readImage();
      if (!clipboardImage.isEmpty()) {
        const imageId = Date.now();
        const imagePath = path.join(tempDir, `clipboard-${imageId}.png`);
        fs.writeFileSync(imagePath, clipboardImage.toPNG());
        const id2 = `image-${imageId}`;
        const queueItem2 = {
          id: id2,
          type: "image",
          content: imagePath,
        ...
        requestsQueue.push(queueItem2);
        mainWindow2.webContents.send("queue-updated", requestsQueue);
        return { success: true, item: queueItem2 };
      }
  ...
  electron.ipcMain.handle("remove-from-queue", (_, itemId) => {
  ...
  electron.ipcMain.handle("clear-queue", () => {
    requestsQueue = [];
    mainWindow2.webContents.send("queue-updated", requestsQueue);
    return { success: true };
  });
  electron.ipcMain.handle("get-queue", () => {
...
electron.app.whenReady().then(() => {
  ...
  registerHotkeys({
    ...
    moveWindow: (direction) => {
      if (mainWindow) {
        const [x2, y] = mainWindow.getPosition();
        const step = 62;
        switch (direction) {
          case "up":
            mainWindow.setPosition(x2, y - step);
            break;
          case "down":
            mainWindow.setPosition(x2, y + step);
            break;
          case "left":
            mainWindow.setPosition(x2 - step, y);
            break;
          case "right":
            mainWindow.setPosition(x2 + step, y);
            break;
